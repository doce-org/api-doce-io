...
var Service = function () {
  ...
  _createClass(Service, [{
        ...
        if (method) {
          if (key === '$or') {
            var _ret = function () {
              ...
              return {
                v: value.forEach(function (condition) {
    ...
    value: function _find(params, count) {
      ...
      var q = this.db().select(['*']);

      // $select uses a specific find syntax, so it has to come first.
      if (filters.$select) {
        var _db;

        q = (_db = this.db()).select.apply(_db, _toConsumableArray(filters.$select));
      }
      ...
      if (filters.$sort) {
        Object.keys(filters.$sort).forEach(function (key) {
          return q = q.orderBy(key, parseInt(filters.$sort[key], 10) === 1 ? 'asc' : 'desc');
        });
      }
    ...
    value: function patch(id, raw, params) {
      ...
      return q.update(data).then(function () {
        return _this4._find({ query: patchQuery }).then(function (page) {
          var items = page.data;
